    ...
    if st.sidebar.button("Process & Import Excel"):
        try:
            ...
            if missing_cols:
                ...
                # STRICT VALIDATION: Filter out rows where Ref is missing/blank/NaN
                valid_ref_mask = imported_df["Ref"].astype(str).str.strip().notna() & (imported_df["Ref"].astype(str).str.strip() != "") & (imported_df["Ref"].astype(str).str.strip().lower() != "nan")
...
# --- OPTION 2: MAIN DASHBOARD & MASTER TABLES (WITH MULTI SEARCH & DELETE) ---
